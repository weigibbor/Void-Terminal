    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          style={{ background: 'var(--base, #0A0A0D)', border: '0.5px solid #2A2A30', borderRadius: '12px', boxShadow: '0 16px 48px rgba(0,0,0,0.5)', padding: '16px 20px', minWidth: '360px' }}
        >
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-[4px]">
            {bindings.slice(0, 16).map(kb => (
              <div key={kb.id} className="flex items-center justify-between py-[2px]">
                ...
                <kbd className="text-[9px] text-void-text-ghost font-mono px-[6px] py-[1px] rounded-[3px]" style={{ background: 'var(--elevated, #141418)', border: '0.5px solid #2A2A30' }}>{format(kb.keys)}</kbd>
              </div>
            ))}
          </div>
